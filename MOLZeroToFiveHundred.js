...
*/
function zeroToFiveHundred() {
    function playGame() {
        alert("Welcome to more or less.. This game is simple. Your mission is to find the randomly generated number between 0 and 500. You only have 7 attempts to find the correct number. Good luck!");
        ...
        const randomNumber = getRandomIntInclusive(0, 500);
...

        {
            ...
            while (playGame && counter != maxAttempts) {
                ...
                let userInput = prompt(`Guess the number between 0 and 500. ${maxAttempts - counter} attempts remaining.`);
                let userNum = parseInt(userInput);
                if (isNaN(userNum)) {
                    alert("That is not a valid number. GAME OVER!");
                } else if (userNum > 500 || userNum <= -1) {
                    alert("That is not a valid number. GAME OVER!");
                    return;
                }
